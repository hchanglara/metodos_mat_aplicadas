...
$$
entonces podemos definir $f(A)$ equivalentemente usando la serie
$$
f(A) = \sum_{k=0}^\infty f_kA^k.
$$
Gracias a la convergencia absoluta de $f(x)$ tenemos la convergencia absoluta de $f(A)$ con respecto a la norma de operador
$$
...
Para justificar estas identidades basta usar que las series convergen de forma absoluta para todo $t\in \mathbb R$. La norma que usamos en este caso es conocida como la **norma de operador**
$$
\|A\| := \sup_{\|x\|=1} \|Ax\|.
$$
...
entonces
$$
u(x,t) = \int_0^L S(x,t,y)v_0(y)dy, \qquad S(x,t,y) := \int_0^t C(x,s,y)ds = \frac{2}{L}\sum_{k=1}^\infty \frac{\sin(k \pi t/L)}{k\pi}\sin(k\pi x)\sin(k\pi y).
$$
...
$$
u(x,t) = \int_0^L W(x,t,y)u_0(y)dy+\int_0^L S(x,t,y)v_0(y)dsdy.
$$
...
$$
\int_{-L}^L \phi_k(x)\overline{\phi_j(x)}dx = \int_{-L}^L e^{i(k-j)\pi x}dx =\begin{cases}
0 \text{ si } k\neq j,\\
2L \text{ si } k=j.
\end{cases}
$$
Por lo tanto tenemos la siugiente fórmula para los coeficientes de Fourier
$$
\hat f_k := \frac{\langle f,\phi_k\rangle}{\|\phi_k\|^2} = \frac{1}{2L}\int_{-L}^L f(x)e^{-ik\pi x}dx.
$$